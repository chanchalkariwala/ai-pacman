        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            if hitsWall == False:
                if (nextx, nexty) in self.corners and (nextx, nexty) not in visited_corners:
                    visited = visited_corners + [(nextx, nexty)]
                    successors.append((((nextx, nexty), visited), action, 0))
                else:
                    successors.append((((nextx, nexty), visited_corners), action, 0))
...
def cornersHeuristic(state, problem):
    ...
    #Find closest corner, then the closest corner from that one and so on.
    ...
    while len(unvisited) > 0:
        ...
        for i in range(len(unvisited)):
            distance = util.manhattanDistance(current_node, unvisited[i])
            if distance < min_distance:
                min_distance = distance
                closest_corner = unvisited[i]
...
class ClosestDotSearchAgent(SearchAgent):
    ...
    def findPathToClosestDot(self, gameState):
        ...
        fringe.push((current_position, actions_list))

        while fringe.isEmpty() == False:
            current_position, actions = fringe.pop()
            ...
            for child in children:
                child_state, direction, cost = child
                new_actions = actions[:]
                new_actions.append(direction)

                fringe.push((child_state, new_actions))
